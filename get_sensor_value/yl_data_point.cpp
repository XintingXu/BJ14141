...
String yl_data_point::ftoa(float val, char resolution) const
{
	...
	for (long i = 0; i < resolution && f_part >= min_val; ++i)
	{
		temp += long(f_part * 10);
		f_part = f_part * 10 - long(f_part * 10);
		min_val *= 10;
	}
	if (temp.length())
	{
...
String yl_data_point::sub_string(const String &str, long from_index, const String &start_str, const String &end_str)
{
	long start_index = str.indexOf(start_str, from_index);
	if (start_index < 0)
	{
		return "";
	}
	start_index += start_str.length();
	long end_index = str.indexOf(end_str, start_index);
